...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TELOXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TELOXMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TELOXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("telox-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("TELOXMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running TELOXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadTELOXMiner() exception");
    } catch (...) {
        LogPrintf("ThreadTELOXMiner() exception");
    }

    LogPrintf("ThreadTELOXMiner exiting\n");
}
