...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if(nHeight <= 15000 && nHeight > 0) {
					nSubsidy = 3 * COIN;
					if(nSubsidy / 100 * 8 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				...
				} else if (nHeight > 50000 && nHeight <= 90000) {
					nSubsidy = 6 * COIN;
					if(nSubsidy / 100 * 8 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 90000 && nHeight <= 250000) {
					nSubsidy = 8 * COIN;
					if(nSubsidy / 100 * 8 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				...
				} else if (nHeight > 500000 && nHeight <= 1000000) { // 475200 => LAST POW BLOCK
					nSubsidy = 7 * COIN;
					if(nSubsidy / 100 * 8 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
